...
/** One cluster's auto-snapshot outcome. `lastSuccess` is absent until the
 *  scheduler has actually taken one — "enabled" alone never proved anything. */
export interface AutoSnapshotCluster {
  snapshots: number;
...
/** The scheduler covers EVERY cluster (ag#104), so its status is keyed by
 *  cluster name. It was previously read as a flat object, which is why the
 *  console could not show that the Assets Engine was snapshotting even though
 *  the API had been reporting it all along. */
export interface AutoSnapshot {
  enabled: boolean;
  intervalMinutes: number;
  clusters?: Record<string, AutoSnapshotCluster>;
}
