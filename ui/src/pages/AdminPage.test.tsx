...
function stubFetch() {
  ...
  const mock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes('/api/admin/status')) {
      ...
      // Per-cluster since ag#104. The Assets Engine is the load-bearing case:
      // it is not backup-capable, so it can never be the selected cluster.
      body = {
        enabled: true,
        intervalMinutes: 30,
        clusters: {
          match: { snapshots: 12, consecutiveFailures: 0, lastSuccess: '2026-07-25T23:28:25Z', secondsSinceLastSuccess: 232 },
          assets: { snapshots: 0, consecutiveFailures: 3, lastError: 'could not find cluster leader' },
        },
      };
...
describe('AdminPage smoke (multi-cluster)', () => {
  ...
  it('lands on Overview with the fleet summary and per-cluster mini-rails', async () => {
    ...
    // Both derived from the process list itself: 8 running, 8 x 256MB.
    expect(screen.getByText('8/8')).toBeTruthy();       // Services running/total
    expect(screen.getByText('2.0 GB')).toBeTruthy();     // total memory
    // One source for the fleet counts: no second endpoint to drift against.
    expect(fetchStub.calls.some(c => c.url.includes('/processes/summary'))).toBe(false);
  ...
  it('stacks a ClusterSection per cluster; capability-gates the AE ops slot', async () => {
    ...
    // Matching engine rail: the ops it has the capability for
    ...
    // Assets rail: Housekeeping/Snapshot are gated off
  ...
  // The 2026-07-25 regression in one test: the Assets Engine had never taken a
  // snapshot, the console said "Enabled", and there was nowhere to see the
  // difference. The AE is not backup-capable, so a per-SELECTED-cluster panel
  // would still hide it — every scheduled cluster has to be listed.
  it('reports auto-snapshot outcomes for every scheduled cluster, including non-backup ones', async () => {
    renderAdmin();
    await screen.findByText('demo');

    fireEvent.click(screen.getByRole('button', { name: 'Backup' }));
    await screen.findByText('Auto-snapshot');

    // The AE cannot be selected for backup...
    const options = screen.getAllByRole('option').map(o => o.textContent);
    expect(options).not.toContain('Assets Engine');

    // ...but its snapshot health is on screen anyway, and it reads as an alarm.
    expect(screen.getByText('Assets Engine')).toBeTruthy();
    const never = screen.getByText('never');
    expect(never.className).toContain('text-sell');
    expect(screen.getByText('3 failed').getAttribute('title')).toBe('could not find cluster leader');

    // The healthy cluster shows a real age rather than a bare "enabled".
  ...
  it('clears a stale operation when an empty progress frame arrives (no stuck rail)', async () => {
    ...
    // buttons for the percent; the match rail's Housekeeping goes away.
    ...
    expect(match.queryByRole('button', { name: /Housekeeping/i })).toBeNull();
    ...
    expect(await match.findByRole('button', { name: /Housekeeping/i })).toBeTruthy();
