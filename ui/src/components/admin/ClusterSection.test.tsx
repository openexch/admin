...
const CAPS_ALL = { snapshot: true, cleanup: true, housekeeping: true, backup: true, separateDriver: true };
...
const assetsBlock = (over: Partial<ClusterBlock> = {}): ClusterBlock => ({
  ...
  capabilities: { snapshot: false, cleanup: true, housekeeping: false, backup: false, separateDriver: true },
...
const handlers = {
  ...
  onHousekeeping: noop, onSnapshot: noop, onViewLogs: noop,
};
...
describe('ClusterSection', () => {
  it('renders a match block: three node cards + the ops rail', () => {
  ...
  // No admin-gateway code path emits `money`, so this is the state on every live
  // box today. The panel must render NOTHING rather than an empty "Ledger
  // Integrity" heading: a headed section reads as coverage to an operator
  // scanning the console, and this check is not running.
  it('renders no ledger-integrity section at all when money is absent', () => {
    renderSection(assetsBlock());
    expect(screen.queryByText(/Ledger Integrity/i)).toBeNull();
    expect(screen.queryByText(/not reported by this build/i)).toBeNull();
    expect(screen.queryByText(/Conservation/i)).toBeNull();
  });
