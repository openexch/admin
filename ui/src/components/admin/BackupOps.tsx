...
/** Compact age for a snapshot's last success: seconds, minutes, then hours. */
...
export function BackupOps({ clusters }: BackupOpsProps) {
  ...
  // Scheduler outcomes for every cluster it targets, ordered as the console
  // orders clusters so the money ledger is never buried under a map's key order.
  const autoSnapshotRows = useMemo(() => {
    const entries = Object.entries(autoSnapshot?.clusters ?? {});
  ...
  }, [autoSnapshot, clusters]);

  const clusterDisplay = (name: string) =>
    (clusters ?? []).find(c => c.name === name)?.display ?? name;
          ...
          <div className="flex flex-col gap-3">
            {autoSnapshot && (
              <div className="flex flex-col gap-2">
                ...
                {/* Every scheduled cluster, not just the backup-capable one
                    selected above: the Assets Engine has no backup dir, so it
                    can never be the selection, and its snapshot health is
                    exactly what nobody could see during the 2026-07-25 outage. */}
                {autoSnapshotRows.length > 0 && (
                  <div className="flex flex-col gap-1 border-t border-hairline pt-2">
                    {autoSnapshotRows.map(([name, s]) => (
                      <div key={name} className="flex items-center justify-between gap-3 text-[12px]">
                        <span className="text-muted">{clusterDisplay(name)}</span>
                        <span className="flex items-center gap-2">
                          {s.consecutiveFailures > 0 && (
                            <span className="text-sell" title={s.lastError}>
                              {s.consecutiveFailures} failed
                            </span>
                          )}
                          {/* "never" is the outage state, not a blank: the AE
                              had taken zero snapshots for its entire existence
                              while the scheduler read "Enabled". */}
                          <span className={`font-mono tabular-nums ${s.lastSuccess === undefined ? 'text-sell' : 'text-text-strong'}`}>
                            {s.lastSuccess === undefined
                              ? 'never'
                              : `${formatAge(s.secondsSinceLastSuccess)} ago`}
                          </span>
                          <span className="text-faint">({s.snapshots})</span>
                        </span>
                      </div>
                    ))}
                  </div>
